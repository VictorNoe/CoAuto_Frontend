    ...
    <v-container>
        ...
        <v-row>
            <v-col cols="12" md="4" class="align-self-center">
                <v-text-field v-model="searchtext" placeholder="Buscar Usuario" outlined prepend-inner-icon="mdi-magnify"/>
            </v-col>
            <v-col cols="12" md="2">
                            ...
                            <tr v-for="item in filteredUsers" :key="item.id_user">
                                ...
                                <td>
                                    <v-avatar size="36" color="grey">
                                        <img v-if="item?.profile_image !== null && item?.profile_image !== ''" :src="item?.profile_image" :alt="item?.name" />
                                        <v-icon dark v-if="item?.profile_image === null || item?.profile_image === ''">mdi-account-circle</v-icon>
                                    </v-avatar>
                                </td>
                                <td>{{ item.name }}</td>
                                <td>{{ item.email }}</td>
                                    ...
                                    <v-menu bottom left :close-on-click="true">
                                        ...
                                        <v-card>
                                            <v-list>
                                                ...
                                                <v-list-item>
                                                    <v-list-item-action>
                                                        <v-btn text x-small @click="desActivateUser(item)"><v-icon left dark>mdi-toggle-switch-off-outline</v-icon>Desactivar</v-btn>
                                                    </v-list-item-action>
                                                </v-list-item>
                                                <v-list-item>
                                                    <v-list-item-action>
        ...
        <EditUser :dialog.sync="editDialog" :userData="selectedUser" @update:dialog="editDialog = $event" @user-updated="handleUserUpdated" />
    </v-container>
</template>
